...
export function useBlockchainUtils() {
  ...
  const initData = async (chainKey: string, batchSize: number = 20) => {
    if (!embeddedWallet) {
  ...
  const getNextTransaction = (chainKey: string): string => {
    ...
    // Refill pool in background when 15 out of 20 are used
    if (pool.currentIndex >= 5 && pool.currentIndex <= pool.transactions.length) {
      console.log("current index: ", pool.currentIndex)
      console.log("pool transactions length ", pool.transactions.length)
      const nextNonce = pool.baseNonce + pool.transactions.length;
      console.log(nextNonce)
      // Now uses cached client, no need to create new one
      preSignBatch(chainKey, nextNonce, 10);  // 10 more instead of 20
    }
